...
<form action="" method="POST" enctype="multipart/form-data">
  ...
  <br>
  <br>
  <button type="submit" class="btn btn-success">Borrar usuario</button>
</form>
...
<?php
    ...
    $check_username = mysqli_query($mysqli,"SELECT * FROM Users WHERE Email ='$Correo'");
    $check_duplicity = mysqli_num_rows($check_username);
    $row = mysqli_fetch_assoc($result);


    
        if($check_duplicity == 1) {     

                        $SQL = mysqli_query($mysqli,"DELETE FROM Users WHERE Email ='$Correo'");
                        //mysqli_commit($SQL); 
                        $check_duplicity = mysqli_num_rows($SQL);
